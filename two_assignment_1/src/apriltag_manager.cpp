...
class ApriltagManagerNode : public rclcpp::Node {
  ...
  private:
    ...
    void on_detection_received(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg) {
      ...
      // il robot non può spostarsi in z, quindi assegno a target la z di odom
      try {
        geometry_msgs::msg::TransformStamped tf_odom_map = 
          tf_buffer_->lookupTransform(MAP_FRAME, ODOM_FRAME, tf2::TimePointZero);
        target_ref_map.pose.position.z = tf_odom_map.transform.translation.z;
      }
      catch (const tf2::TransformException & ex) {
